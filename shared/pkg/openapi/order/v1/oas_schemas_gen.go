...
type OrderDto struct {
	...
	OrderUUID uuid.UUID `json:"order_uuid"`
	// UUID пользователя, создавшего заказ.
	UserUUID uuid.UUID `json:"user_uuid"`
	// Список UUID деталей в заказе.
	PartUuids []uuid.UUID `json:"part_uuids"`
	// Итоговая стоимость заказа.
	TotalPrice float64 `json:"total_price"`
	// UUID транзакции (присутствует после оплаты).
...
func (s *OrderDto) GetTransactionUUID() NilUUID {
...
func (s *OrderDto) GetOrderStatus() OrderStatus {
...
func (s *OrderDto) SetTransactionUUID(val NilUUID) {
...
func (s *OrderDto) SetOrderStatus(val OrderStatus) {
	s.OrderStatus = val
}
...
type PayOrderResponse struct {
	// Уникальный UUID транзакции оплаты.
